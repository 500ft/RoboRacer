...
**Status: IN PROGRESS.** This is the MechE centerpiece of the portfolio and **absorbs the old item 10 (LiDAR mast package)**. **Design-only: no fabrication.** The author does the CAD and FEA. **The LiDAR-mast analysis is complete on the FIRMED item-15 tip mass:** the LiDAR is now locked (**Hokuyo UST-10LX, 130 g datasheet → mast tip mass 0.175 kg**, item 15 §1.1–§1.2), and the hand calc (§3.1), frequency-fix design revision (§3.2), and gmsh + CalculiX FEA (§4, §6) have all been **re-run on the firmed 0.175 kg mass** (validated within ±15%). The lighter real LiDAR (0.175 vs the old 0.20 kg placeholder) **raised** the frequency margin and improved the strength SF. **Update 2026-07-07:** items 13–15 are now fully locked (competition-rules constraints + all part picks), so the §2 mass budget is filled (≈ 4.42 kg vs the ≤ 4.5 kg R-11 budget) and the §7 tolerance stack's ASSUMED tolerances can be replaced at CAD. Remaining in this file: the parametric CAD (mast + deck interface, transponder bay per R-14) and per-component CG locations from it.
