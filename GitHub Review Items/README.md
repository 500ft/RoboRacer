...
**Status:** **Done (2026-07-07, commit `6285197`).** All numeric requirements set from the sim envelope + model params; the official RoboRacer restricted-class ruleset ([roboracer_rules](https://github.com/f1tenth/roboracer_rules)) folded in as constraints R-13..R-20 (baseline chassis, 8x12 cm transponder bay, onboard-only compute, <=4S drive battery, motor/LiDAR ceilings) with per-pick compliance.
...
**Status:** **Done (2026-07-07, commit `6285197`).** Platform = Traxxas Slash 4x4 VXL (TRA68086-4), the named restricted-class baseline; stock wheelbase 0.324 m documented as a -1.9% deviation vs the 0.3302 m model lock (sim `C_Sf`/`C_Sr` never carried to hardware). Drivetrain sized at the measured 9.51 m/s^2 envelope (~79 A transient vs VESC 6 MkVI 80/120 A); stock 2075 servo clears the 3.2 rad/s steering-rate limit at 2.0x.
...
**Status:** **Done (2026-07-07, commit `6285197`).** LiDAR Hokuyo UST-10LX (locked earlier); IMU = VESC 6 MkVI onboard (BNO055-class fallback flagged); odometry = VESC ERPM via `vesc_to_odom`; compute = Jetson Orin NX 16GB + Seeed J401; power = two Traxxas 2872X 3S packs with a 12 V regulator for the LiDAR only (~30 min drive / ~86 min compute vs the 20 min R-09 target).
...
**Status:** **Analysis complete; parametric CAD remains.** Mast package done end-to-end (telemetry-derived load case superseding the 4g placeholder below; hand calc; modal FAIL -> redesign -> converged gmsh+CalculiX FEA within +-15%; mesh convergence PASS). Mass budget filled at ~4.42 kg vs the <=4.5 kg R-11 budget. Tolerance stack re-run 2026-07-07 at the chassis-informed deck height: blind assembly FAILS -> derived one-time scan-plane leveling step + a **deck-height <= 0.138 m CAD requirement** (the earlier >2x margin claim did not survive the chassis lock and was corrected). Remaining: mast + deck-interface CAD (must hold the deck-height limit and the R-14 transponder bay), which fills the CG budget and replaces ASSUMED tolerances.
